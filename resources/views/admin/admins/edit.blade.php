...
<div class="card">
  <div class="card-header">
    <h5>Edit Admin User: {{ $admin->full_name }}</h5>
    ...
    <form action="{{ route('admin.admins.update', $admin->id) }}" method="POST">
      ...
      <div class="mb-3">
        ...
        <select class="form-control @error('user_role') is-invalid @enderror" 
                ...
                name="user_role" 
                required>
          <option value="admin" {{ old('user_role', $admin->user_role) == 'admin' ? 'selected' : '' }}>Administrator (Full Access)</option>
          <option value="editor" {{ old('user_role', $admin->user_role) == 'editor' ? 'selected' : '' }}>Editor (Limited Access)</option>
          <option value="viewer" {{ old('user_role', $admin->user_role) == 'viewer' ? 'selected' : '' }}>Viewer (Read Only)</option>
        </select>
        @error('user_role')
          <div class="invalid-feedback">{{ $message }}</div>
        @enderror
